...
func NewMigrator() *Migrator {
	...
	migrator := &Migrator{
		...
		DB:       dbconnect.Std(),
		Migrator: dbconnect.Std().Migrator(),
	}
...
func (migrator *Migrator) Rollback() {
	...
	if !migrator.rollbackMigrations(migrations) {
		output.Success("[migrations] table is empty, nothing to rollback.")
	}
}
...
func (migrator *Migrator) rollbackMigrations(migrations []Migration) bool {
	...
	for _, _migration := range migrations {
		...
		// 友好提示
		output.Warning("rollback " + _migration.Migration)

		// 执行迁移文件的 down 方法
		mFile := getMigrationFile(_migration.Migration)
		if mFile.Down != nil {
			mFile.Down(dbconnect.Std().Migrator(), dbconnect.SQLDB)
		}
		...
		// 打印运行状态
		output.Success("finsh " + mFile.FileName)
	}
	return runed
}
...
func (migrator *Migrator) readAllMigrationFiles() []MigrationFile {
	...
	for _, f := range files {
		...
		// 去除文件后缀 .go
		fileName := arms.FileNameWithoutExtension(f.Name())
...
func (migrator *Migrator) runUpMigration(mfile MigrationFile, batch int) {
	...
	if mfile.Up != nil {
		// 友好提示
		output.Warning("migrating " + mfile.FileName)
		// 执行 up 方法
		mfile.Up(dbconnect.Std().Migrator(), dbconnect.SQLDB)
		// 提示已迁移了哪个文件
		output.Success("migrated " + mfile.FileName)
	}
...
func (migrator *Migrator) Reset() {
	...
	// 回滚所有迁移
	if !migrator.rollbackMigrations(migrations) {
		output.Success("[migrations] table is empty, nothing to reset.")
	}
}
...
func (migrator *Migrator) Fresh() {
	...
	// 获取数据库名称，用以提示
	dbname := dbconnect.CurrentDatabase()

	// 删除所有表
	err := dbconnect.DeleteAllTables()
	output.ExitIf(err)
	output.Success("clearup database " + dbname)

	// 重新创建 migrates 表
	migrator.createMigrationsTable()
	output.Success("[migrations] table created.")
